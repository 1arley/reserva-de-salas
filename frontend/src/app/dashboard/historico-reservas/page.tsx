...
export default function ReservationsHistoryPage() {
    ...
    const loadReservations = useCallback(async () => {
        ...
        setError(null)
        try {
            const result = await reservationsApi.history({
    ...
    async function handleExport() {
        ...
        try {
            ...
            link.href = url
            link.download = `historico-reservas-${new Date().toISOString().slice(0, 10)}.csv`
    ...
    return (
        <div className='space-y-6'>
            <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
                <div>
                    <h1 className='text-2xl font-semibold text-slate-900'>Histórico de Reservas</h1>
                    <p className='mt-1 text-sm text-slate-500'>
                        Visualizar reservas passadas e canceladas.
                    </p>
                </div>
                <Button
                    variant='outline'
